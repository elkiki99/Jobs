<form wire:submit.prevent="updateCertifications">
    <h2 class="pb-4 text-4xl">{{ __('Certifications') }}</h2>
    @foreach ($certifications as $index => $certification)
        <div class="space-y-2">
            <x-input-label for="certifications" :value="__('Certification title *')" />
            <x-text-input placeholder="Your certification title" wire:model="certifications.{{ $index }}.title"
            ...
            <x-input-error class="mt-2" :messages="$errors->get('certifications.' . $index . '.date')" />
        </div>
    ...
    @endforeach
    <x-primary-button class="my-4" wire:click.prevent="updateCertifications">
        Save certifications
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
            stroke="currentColor" class="ml-1 size-4">
            <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <x-secondary-button type="button" wire:click="addCertification">
        Add certifications
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
            stroke="currentColor" class="ml-1 size-4">
    ...
    @if (session('certifications_updated'))
        <div x-data="{ show: true }" x-init="setTimeout(() => show = false, 2000)" x-show="show"
            ...
            {{ session('certifications_updated') }}
        </div>
    @endif
    
    @if (session('certifications_updated_error'))
        <div x-data="{ show: true }" x-init="setTimeout(() => show = false, 2000)" x-show="show"
            x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
            x-transition:enter-end="opacity-100" x-transition:leave="transition ease-out duration-300"
            x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="mb-4 text-red-600">
            {{ session('certifications_updated_error') }}
        </div>
    @endif
</form>
