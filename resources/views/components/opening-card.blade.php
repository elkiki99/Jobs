            ...
            <div class="flex gap-2">
                <div class="flex items-center gap-6">
                    <a wire:navigate href="{{ route('companies.show', $opening->company->slug) }}"
                        class="text-gray-400 hover:underline">{{ $opening->company->name }}
                    </a>
                    <p class="my-2 text-sm text-gray-500">${{ number_format($opening->salary, 2) }}</p>
                </div>
                <a wire:navigate href="{{ route('openings.show', $opening->slug) }}" class="ml-auto">
