<x-app-layout>
    ...
    <div class="px-4 mx-auto sm:px-6 lg:px-8 max-w-7xl">
        <p class="pb-12 text-lg text-gray-600 sm:text-2xl">{{ $company->industry }}</p>
        ...
        <div class="flex-row gap-8 lg:flex">
            <div class="w-full space-y-2 lg:w-1/3 ">
                @if ($company->logo)
                    <img class="object-cover rounded-full size-36 aspect-square"
                        ...
                        alt="{{ $company->name }}">
                @else
                    <img class="object-cover rounded-full size-36 aspect-square" src="{{ asset('images/no-image.png') }}"
                        alt="{{ $company->name }}">
                @endif
                ...
                <div class="flex items-center gap-2">
                    ...
                    <a wire:navigate href="https://www.google.com/maps/search/{{ urlencode($company->address) }}"
            ...
            <div class="flex flex-col w-full min-h-screen lg:w-2/3">
                ...
                <div class="mt-5">
                    ...
                    @endif
                    @if (session('company_updated_error'))
                        <div x-data="{ show: true }" x-init="setTimeout(() => show = false, 2000)" x-show="show"
                            x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
                            x-transition:enter-end="opacity-100" x-transition:leave="transition ease-out duration-300"
                            x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
                            class="mb-4 text-red-600">
                            {{ session('company_updated_error') }}
                        </div>
                    @endif
